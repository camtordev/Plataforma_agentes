...
const ICON_APPLE_LEAF =
  "M10 2c1 0 2 .5 2 2a2 2 0 0 1-2 2 2 2 0 0 1-2-2c0-1.1.9-2 2-2Z";
...
const GridCanvas = ({ activeBrushItem }) => {
  ...
  const handleDraw = (stage, x, y) => {
    ...
    if (selectedTool === "eraser") {
      ...
      if (item.type === "obstacle")
        sendMessage({
          type: "ADD_OBSTACLE",
          data: { x, y, subtype: item.subtype, config: item.config },
        });
      else if (item.type === "resource")
        sendMessage({
          type: "ADD_FOOD",
          data: { x, y, food_type: item.subtype, config: item.config },
        });
      else if (item.type === "agent")
        sendMessage({
          type: "ADD_AGENT",
          data: { x, y, agent_type: item.subtype, config: item.config },
        });
    }
  };
  ...
  const handleMouseDown = (e) => {
    ...
    const isClickingSelected =
      clickedAgent && selectedAgentIds.includes(clickedAgent.id);
  ...
  const handleMouseMove = (e) => {
    ...
    if (isMovingGroup.current) {
      ...
      if (dragStart.current.maxX + dx >= width)
        dx = width - 1 - dragStart.current.maxX;

      // Lo mismo para Y
      if (dragStart.current.minY + dy < 0) dy = -dragStart.current.minY;
      if (dragStart.current.maxY + dy >= height)
        dy = height - 1 - dragStart.current.maxY;
  ...
  const handleStageClickLogic = (e) => {
    ...
    if (clickedAgent) {
      ...
      const agentType = clickedAgent.type
        ? clickedAgent.type.toLowerCase()
        : "reactive";
  ...
  const renderSelectionOverlay = () => {
    ...
    return (
      <Rect
        x={minX * actualCellSize}
        y={minY * actualCellSize}
        width={widthCells * actualCellSize}
        height={heightCells * actualCellSize}
    ...
    >
      <Layer>
        ...
        {Array.from({ length: width + 1 }).map((_, i) => (
          <Line
            key={`v-${i}`}
            points={[i * actualCellSize, 0, i * actualCellSize, canvasHeight]}
            stroke="#27272a"
            strokeWidth={1}
          />
        ))}
        {Array.from({ length: height + 1 }).map((_, i) => (
          <Line
            key={`h-${i}`}
            points={[0, i * actualCellSize, canvasWidth, i * actualCellSize]}
            stroke="#27272a"
            strokeWidth={1}
          />
        ))}
        ...
        {obstacles.map((obs, i) => {
          ...
          return (
            <Rect
              key={`o-${i}`}
              x={obs.x * actualCellSize}
              y={obs.y * actualCellSize}
        ...
        {food.map((f, i) => {
          ...
          return (
            <Group
              key={`f-${i}`}
              x={f.x * actualCellSize}
              y={f.y * actualCellSize}
            >
              <Circle
                x={actualCellSize / 2}
                y={actualCellSize / 2}
        ...
        {agents.map((a, i) => {
          ...
          return (
            <Group
              key={`a-${i}`}
              x={a.x * actualCellSize}
              y={a.y * actualCellSize}
            >
              {isSelected && (
                <Rect
                  x={0}
              ...
              <Rect
                ...
                width={Math.max(
                  0,
                  ((actualCellSize - 4) * (a.energy || 100)) / 100
                )}
                height={3}
