...
import { Activity, Apple, Zap, Users, Clock, Footprints, History } from "lucide-react"
    ...
    <div className="flex flex-col h-full font-sans text-zinc-100 bg-zinc-900 w-full">
      ...
      <div className="flex bg-zinc-950 p-1 border-b border-zinc-800 shrink-0 gap-1">
        <button 
            ...
            className={`flex-1 text-xs py-2 rounded-md font-medium transition-all ${activeTab === 'general' ? 'bg-zinc-800 text-blue-400 shadow-sm' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
        >
        ...
        <button 
            ...
            className={`flex-1 text-xs py-2 rounded-md font-medium transition-all ${activeTab === 'agents' ? 'bg-zinc-800 text-blue-400 shadow-sm' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
        >
             <div className="flex items-center justify-center gap-2">
        ...
        {activeTab === "general" ? (
          ...
          <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="p-3 rounded-lg bg-zinc-800/50 border border-zinc-700/50 flex justify-between items-center shadow-sm">
                <div>
                    <p className="text-[10px] uppercase text-zinc-500 font-bold tracking-wider">Tick Actual</p>
                    <p className="text-xl font-mono text-white tracking-widest">{step}</p>
                </div>
          ...
          <div className="space-y-3">
             <div className="flex items-center justify-between mb-2">
                <h3 className="text-[10px] font-bold text-zinc-500 uppercase">Historial por Agente</h3>
                <span className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full">{totalAgents} Agentes</span>
             </div>

             {agents.map((agent) => (
                <div key={agent.id} className="bg-zinc-800/30 border border-zinc-800 rounded-lg p-3 text-xs hover:border-zinc-700 transition-colors">
                    {/* Cabecera Agente */}
                    <div className="flex justify-between items-center mb-2 border-b border-zinc-800/50 pb-2">
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full shadow-[0_0_5px]" style={{ background: agent.color, boxShadow: `0 0 5px ${agent.color}` }}></div>
                            <span className="font-bold text-zinc-200 uppercase">{agent.type}</span>
                            <span className="text-zinc-600 font-mono text-[10px]">#{agent.id.split('_')[1]}</span>
                        </div>
                    ...
                    <div className="font-mono text-[10px] text-zinc-400 break-words leading-relaxed bg-black/30 p-2 rounded border border-zinc-800/50">
                        {agent.path && agent.path.length > 0 ? (
                            agent.path.slice(-8).map((pos, idx) => (
                                <span key={idx} className={idx === agent.path.slice(-8).length - 1 ? "text-yellow-200 font-bold" : ""}>
                                    {idx > 0 && " → "}
                                    [{pos[0]},{pos[1]}]
                                </span>
                            ))
  ...
  <div className={`p-4 rounded-lg border ${borderColor} ${bgColor} transition-all hover:brightness-110`}>
    <div className="flex justify-between items-start">
      <div>
        ...
        {subtext && <p className="text-[10px] text-zinc-500 mt-0.5 opacity-80">{subtext}</p>}
      </div>
      <div className={`p-2 rounded-md bg-zinc-900/40 backdrop-blur-sm ${color}`}>
        <Icon size={18} />
      </div>
    </div>
  </div>
